...
bool cl_init_device(char *kernelname)
{
  ...
  if(numPlatforms>0)
  {
    cl_platform_id* platforms = (cl_platform_id *) malloc(numPlatforms * sizeof(cl_platform_id)); // FIXED
    // get platforms
  ...
  {
      char * content;// = zetaperft_cl;
      size_t len;// = zetaperft_cl_len;
      {
        FILE* fileKernel = fopen("zetaclperft.h", "r");
        if (!fileKernel)
        {
            fprintf(stderr, "Failed to load zetaperft_cl.\n");
            return false;
        }
...
#define MAX_SOURCE_SIZE (0x100000)
        ...
        len = fread(content, 1, MAX_SOURCE_SIZE, fileKernel);
        fclose(fileKernel);
      }
  ...
  if (strstr(kernelname, "alphabeta_gpu"))
  {
      ...
      */
      char * content;// = zeta_cl;
      size_t len;// = zeta_cl_len;
      {
          FILE* fileKernel = fopen("zetacl.h", "r");
          if (!fileKernel)
          {
              fprintf(stderr, "Failed to load zetacl.\n");
              return false;
          }
